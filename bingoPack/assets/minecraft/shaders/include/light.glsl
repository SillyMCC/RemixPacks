...
vec4 minecraft_sample_lightmap_custom(sampler2D lightMap, ivec2 uv, float faceLighting) {
    #ifdef CUSTOM_LIGHTING
    faceLighting = mix(faceLighting, 1.0, uv.x / 256.0);
    #endif
    return texture(lightMap, clamp(uv / 256.0, vec2(0.5 / 16.0), vec2(15.5 / 16.0))) * faceLighting;
}